...
export default function ReportsPage() {
  ...
  // Set strictly to 25 records per page (Screen & PDF)
  ...
  // Fetch ALL records via parallel Chunk Requests
  ...
  return (
    <div className="flex h-screen bg-[#f0f4f8] font-sans text-gray-800 antialiased overflow-hidden relative print:h-auto print:overflow-visible print:bg-white">
      ...
      {/* 25 Records Compact Print Styling */}
      <style jsx global>{`
        @media print {
          @page {
            size: A4 portrait;
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full z-10 relative print:overflow-visible">
        ...
        <div className="hidden print:block mb-2 border-b-2 border-gray-800 pb-1.5">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <p className="text-[8.5px] text-gray-500">Page: {currentPage} of {totalPages}</p>
              {(searchQuery || selectedStatus !== 'All') && (
                <p className="text-[8.5px] font-bold text-teal-700 mt-0.5">
                  Filtered: {searchQuery ? `"${searchQuery}" ` : ''}{selectedStatus !== 'All' ? `[${selectedStatus}]` : ''}
                </p>
              )}
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 pt-6 print:p-0 print:overflow-visible">
          ...
          <div className="print:hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs md:text-sm text-gray-500">25 Records per view layout for optimal A4 print fit.</p>
            </div>
          ...
          {/* Table Container - Exactly 25 records per view */}
          <div className="bg-white/50 backdrop-blur-2xl rounded-[2rem] border border-white/70 shadow-xs overflow-hidden print:bg-white print:rounded-none print:border-none print:shadow-none">
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left text-xs md:text-sm min-w-[700px] print:min-w-full print:table">
                <thead className="text-[11px] md:text-xs uppercase font-bold text-gray-500 bg-white/40 border-b border-gray-200 print:bg-gray-100 print:text-gray-900">
                  <tr>
                    ...
                    <th className="py-2.5 px-3 text-right">Gear Price</th>
                    <th className="py-2.5 px-3 text-right">TC Amt</th>
                    <th className="py-2.5 px-3 text-center">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/60 font-medium print:divide-gray-300">
                  {loading ? (
                    <tr><td colSpan="8" className="text-center py-10 text-gray-400">Loading records...</td></tr>
                    ...
                    currentRecords.map(o => (
                      <tr key={o.id} className="hover:bg-white/40 transition">
                        <td className="py-2 px-3 text-gray-600 print:py-0.5">{o.order_date || '—'}</td>
                        <td className="py-2 px-3 font-bold text-[#3db2a8] print:text-gray-900 print:py-0.5">#{o.id}</td>
                        <td className="py-2 px-3 font-bold text-[#1a2b3c] print:text-gray-900 print:py-0.5">{o.customers?.name || 'Customer Deleted'}</td>
                        <td className="py-2 px-3 text-gray-700 print:py-0.5">
                          {o.model || 'Standard'} (OD: {o.od || '—'}, NT: {o.nt || '—'})
                        </td>
                        <td className="py-2 px-3 text-center font-bold text-teal-700 print:text-gray-900 print:py-0.5">{o.qty || 1}</td>
                        <td className="py-2 px-3 text-right font-bold text-gray-900 print:py-0.5">₹{Number(o.gear_price || 0).toLocaleString('en-IN')}</td>
                        <td className="py-2 px-3 text-right text-gray-600 print:py-0.5">₹{Number(o.tc_amt || 0).toLocaleString('en-IN')}</td>
                        <td className="py-2 px-3 text-center print:py-0.5">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold print:border print:px-1 print:py-0 ${getStatusBadge(o.status)}`}>
                            {o.status}
                          </span>
                        </td>
                      </tr>
          ...
          {/* PRINT ONLY: Footer (Balanced immediately below 25th row) */}
          <div className="hidden print:flex justify-between items-end pt-3 mt-3 border-t border-gray-300 text-xs">
            <div>
              <p className="font-bold text-gray-800">KHAKARE ENGINEERING</p>
              <p className="text-[9px] text-gray-500">A4 Production Statement - Page {currentPage} of {totalPages}</p>
            </div>
